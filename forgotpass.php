...
$motp='123456';
        if($otp == $motp)
        {
                  ...
                  $result1 = $con->query("select * from login WHERE `l_email`='$email' ");
                               while($row1= mysqli_fetch_array($result1))
                                 {
                   $type= $row1['l_usertype'];
                                    }
                   
$sql1="UPDATE `login` SET l_password='$pwd' WHERE `l_email`='$email'";
...
switch ($type) {
  case "Architecture":
   ...
   $sql2="UPDATE `company_registration` SET c_password='$pwd' WHERE `c_email`='$email'";
mysqli_query($con,$sql2);
 echo "<script>window.location.replace('login.php');</script>";
    break;
  case "realestate broker":
    $sql2="UPDATE `real_estate_registration` SET r_password='$pwd' WHERE `r_email`='$email'";
mysqli_query($con,$sql2);
 echo "<script>window.location.replace('login.php');</script>";
    break;
  case "user":
     $sql2="UPDATE `userregistration` SET u_password='$pwd' WHERE `u_email`='$email'";
mysqli_query($con,$sql2);
 echo "<script>window.location.replace('login.php');</script>";
    break;
  default:
...
$sql2="UPDATE `login` SET l_password='$pwd' WHERE `l_email`='$email'";
mysqli_query($con,$sql2);
        }
        else
        {
              ...
              <div class="row justify-content-between md:justify-content-center align-items-center">
                <div class="col-lg-9 col-md-10">
                  <div data-anim="slide-up delay-1">
                    ...
                    <div class="page-masthead__back_title">Forgot Password</div>
                    <h1 class="page-masthead__title text-white">Forgot Password</h1>
                  </div>
                </div>
                ...
                <!-- <div class="col-auto">
                  <div data-anim="slide-up delay-1" class="page-masthead-bread text-white md:mt-24">
                    ...
                    /
                    <a data-barba href="#" class="page-masthead-bread__item ">Forgot Password</a>
                  </div>
                </div> -->
              </div>
            </div>
              ...
              <div class="col-xl-7 col-lg-7 offset-lg-5 z-5">
                <div class="mt-minus-xl md:mt-0">
                  <div class="sectionHeading -left-line bg-dark py-48 px-48 md:py-20 md:px-20">
                    ...
                    Login Now</span>
                    <h2 class="sectionHeading__title text-white">Create New Password</h2>
